...
final class InvoiceController
{
    ...
    public function show(string $id): void
    {
        ...
        View::render('invoices/show',['title'=>$invoice['invoice_number'],'invoice'=>$invoice,'items'=>$this->items((int)$invoice['id']),'payments'=>$p->fetchAll(),'flash'=>$this->pullFlash(),'bank'=>$this->bank(),'agency'=>$this->agency(),'canManage'=>true,'stripeEnabled'=>false]);
    }
    ...
    public function portalShow(string $id): void
    {
        ...
        View::render('invoices/show',['title'=>$invoice['invoice_number'],'invoice'=>$invoice,'items'=>$this->items((int)$invoice['id']),'payments'=>$p->fetchAll(),'flash'=>$flash,'bank'=>$this->bank(),'agency'=>$this->agency(),'canManage'=>false,'stripeEnabled'=>(new StripeClient())->enabled()]);
    }
    ...
    public function status(string $id): never
    {
        ...
        else{Database::connection()->prepare('UPDATE invoices SET status=:status WHERE id=:id')->execute(['status'=>$status,'id'=>$invoice['id']]);if($status==='sent')(new MailService())->invoice((int)$invoice['id']);ActivityService::log('invoice.status','invoice',(int)$invoice['id'],$invoice['invoice_number'].' changed to '.$status);$this->flash('success','Invoice status updated.');}
    ...
    private function agency(): array{return ['name'=>Setting::get('agency_name','Veelox Digital'),'logo'=>Setting::get('agency_logo',''),'email'=>Setting::get('agency_email',''),'phone'=>Setting::get('agency_phone',''),'address'=>Setting::get('agency_address',''),'footer'=>Setting::get('invoice_footer','Thank you for your business.')];}
    private function emptyInvoice(): array{$days=max(1,(int)Setting::get('invoice_due_days','14'));return ['customer_id'=>'','order_id'=>'','status'=>'draft','issue_date'=>date('Y-m-d'),'due_date'=>date('Y-m-d',strtotime('+'.$days.' days')),'notes'=>''];}
